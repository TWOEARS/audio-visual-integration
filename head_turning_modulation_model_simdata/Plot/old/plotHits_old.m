function plotHits (htm, vec)
    ...
    l = mergeLabels(p.AVPairs);
    for iLabel = 1:nb_labels
        idx = find(strcmp(l, labels{iLabel}));
        r = p.repartition;
        s = p.scenario;
        s = s.scene{1};
        tt = find(s == idx);
        tt = r{tt}(1);
        tmIdx = [];
        tmp = find(strcmp(htm.gtruth{tt}(:, 1), labels{iLabel}));
        %tmp = find(strcmp(getObject(htm, 'all', 'label'), labels{iLabel}));
        data{iLabel} = htm.gtruth_data{tt}(:, tmp);
        ii = 1;
        while ii <= size(data{iLabel}, 2)
            if sum(data{iLabel}(1:p.nb_audio_labels, ii)) ~= 0 && sum(data{iLabel}(p.nb_audio_labels+1:end, ii)) ~= 0
                ii = ii + 1;
            else
                data{iLabel}(:, ii) = [];
            end
