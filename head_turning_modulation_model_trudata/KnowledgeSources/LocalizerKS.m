% No further explanations required, this KS is highly similar to the
% LocationKS found in the standard blackboard system. Only modification:
%   the signal energy condition is taken from the SignalLevel
%   the invocation frequency is now set to continuous triggering


classdef LocalizerKS < AuditoryFrontEndDepKS
    ...
    methods
        function obj = LocalizerKS(gmName, angularResolution, bTrain)
            param = genParStruct(...
                'fb_type', 'gammatone', ...
                'fb_lowFreqHz', 80, ...
                'fb_highFreqHz', 8000, ...
        ...
        function [bExecute, bWait] = canExecute(obj)
            ...
            bExecute = signalEnergy;%hasSignalEnergy(timeSObj, obj.blocksize_s, obj.timeSinceTrigger);
            bWait = false;
        end

        function execute(obj)
            fprintf('LocalizerKS Evaluation\n') ;
            afeData = obj.getAFEdata();
            ildsSObj = afeData(1);
